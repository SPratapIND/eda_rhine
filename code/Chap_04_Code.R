...
#merge the runoff classes with the rest of information in runoff_summary
to_merge <- runoff_stats_class[, .(sname, runoff_class)]
runoff_summary <- runoff_stations[to_merge, on = 'sname']

#Aggregation (by mean)- daily time scale changes to monthly, seasonal and annual scales
...
#seasonality estimation using boxplot
ggplot(runoff_month, aes(x = factor(month), y = value)) +
  geom_boxplot(fill = colset_4[4]) +
  facet_wrap(~sname, scales = 'free') + 
  theme_bw()

#seasonality changes estimates by plot the annual or the seasonal means.
...
#Runoff consistancy with catchment size
