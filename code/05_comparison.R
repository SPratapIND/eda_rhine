...
lm_plot <- runoff_stats[runoff_summary[, .(sname, area, category)], on = 'sname']
ggplot(lm_plot, aes(x = factor(mean_day), y = area)) +
...
ggplot() +
  geom_point(data = lm_plot, aes(x = mean_day, y = area, col = category), cex = 3) +
  geom_smooth(data = lm_plot[c(1:7)], aes(x = mean_day, y = area), 
              method = 'lm', formula = y~x, se = 0, col = colset_4[1]) +
  geom_smooth(data = lm_plot[c(8:11)], aes(x = mean_day, y = area), 
              method = 'lm', formula = y~x, se = 0, col = colset_4[1]) +
  geom_smooth(data = lm_plot[c(12:17)], aes(x = mean_day, y = area), 
              method = 'lm', formula = y~x, se = 0, col = colset_4[1]) +
  scale_color_manual(values = colset_4[c(2, 3, 4)]) +
  xlab(label = "Area (km3)") +
  ylab(label = "Runoff (m3/s)") +
